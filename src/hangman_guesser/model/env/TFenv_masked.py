...
class HangmanEnvironment(py_environment.PyEnvironment):
    ...
    def __init__(
        ...
        # )
        self._observation_spec = {'observations': array_spec.BoundedArraySpec( shape=(1,30), dtype=np.float32, minimum=-1, maximum=26, name="game"),
                                  'legal_moves': array_spec.ArraySpec(shape=(26,), dtype=np.float32),}   
    ...
    def _success(self, action):
        ...
        observations_and_legal_moves = {'observations': np.array([self._state], dtype=np.float32),
    ...
    def _reset(self):
        ...
        observations_and_legal_moves = {'observations': np.array([self._state], dtype=np.float32),
                                        'legal_moves': legal_moves}
        return ts.restart(observation=observations_and_legal_moves)
    ...
    def _format_legal_moves(self, forbidden_moves, action_dim=26):
        ...
        """
        new_legal_moves = np.full(action_dim, 0.,dtype=np.float32)
        if forbidden_moves:
            new_legal_moves[forbidden_moves] =  -float('inf')
